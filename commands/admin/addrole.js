...
module.exports.run = async (client, message, args) => {
  ...
  if (rMember === message.author.id)
    return message.channel.send(
      "<a:b_no:721969465205588048> | You can't addrole to urself"
    );
  if (rMember === client.user.id)
    return message.channel.send(
      "<a:b_no:721969465205588048> | You can't addrole to me"
    );

  let role = message.guild.roles.find || message.mentions.roles.first();
  if (!role)
    return message.channel.send(
      "<a:b_no:721969465205588048> | Please mention the roles."
    );
  ...
  if (rMember.roles.cache.has(role.id)) {
    return message.channel.send(
      `<a:b_no:721969465205588048> | **${rMember.displayName}**, already has the role!`
    );
  } else {
    await rMember.roles
      .add(role.id)
