  ...
  if (db.has(`${message.author.id}`, "fishing rod") === true) {
    ...
    if (fished != null && timeoutfished - (Date.now() - fished) > 0) {
      ...
      message.channel.send(
        `You have already fished please come back in **${time.hours}h ${time.minutes}m ${time.seconds}s**`
      );
    } else {
      ...
      let embed = new Discord.MessageEmbed()
        ...
        .setDescription(
          `${message.author}, you caught a ${job} and earnt 💴 ${amountearned} Credits`
        );
    ...
    message.channel.send(
      "You need to buy a fishing rod first (hint: _shop leads you to shop where you can buy stuff)"
    );
  }
};
...
exports.help = {
  name: "fish",
  description: "fishing and got money",
  usage: "fish",
  example: "/fish"
};
