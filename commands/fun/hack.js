...
exports.run = async (client, message, args, ops) => {
  if (!args[0])
    return message.channel.send(
      "<:redtick:719865119277842492> | Please mention the users!"
    );
  
  const toHack = args.slice(0).join(" ") && args.shift().toLowerCase();
  message.delete();
  
  if(toHack.id === "583649910092595232") return message.channel.send("You can't hack my owner!");
